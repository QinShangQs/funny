function QiniuJsSDK() {
	...
	this.getFileExtension = function(filename) {
		...
		if (tempArr.length === 1 || (tempArr[0] === "" && tempArr.length === 2)) {
			...
			ext = tempArr.pop().toLowerCase(); // get the extension and make it
												// lower-case
		}
		return ext;
	};
	...
	this.uploader = function(op) {
		...
		var reset_chunk_size = function() {
			...
			var isSpecialSafari = (mOxie.Env.browser === "Safari"
					&& mOxie.Env.version <= 5 && mOxie.Env.os === "Windows" && mOxie.Env.osVersion === "7")
					|| (mOxie.Env.browser === "Safari"
							&& mOxie.Env.os === "iOS" && mOxie.Env.osVersion === "7");
			if (ie && ie <= 9 && op.chunk_size
					&& op.runtimes.indexOf('flash') >= 0) {
				// link:
				// http://www.plupload.com/docs/Frequently-Asked-Questions#when-to-use-chunking-and-when-not
				// when plupload chunk_size setting is't null ,it cause bug in
				// ie8/9 which runs flash runtimes (not support html5) .
				op.chunk_size = 0;
		...
		uploader
				.bind(
						...
						function(up, file) {
							...
							var chunk_size = up.getOption
									&& up.getOption('chunk_size');
							chunk_size = chunk_size
									|| (up.settings && up.settings.chunk_size);
							if (uploader.runtime === 'html5' && chunk_size) {
								if (file.size < chunk_size) {
									directUpload(up, file, that.key_handler);
								} else {
									var localFileInfo = localStorage
											.getItem(file.name);
							...
							return function(up, file, info) {
								...
								var last_step = function(up, file, info) {
									if (op.downtoken_url) {
										...
										ajax_downtoken.open('POST',
												op.downtoken_url, true);
										ajax_downtoken
												.setRequestHeader(
														'Content-type',
										...
										ajax_downtoken.onreadystatechange = function() {
											if (ajax_downtoken.readyState === 4) {
												if (ajax_downtoken.status === 200) {
													...
													try {
														res_downtoken = that
																.parseJSON(ajax_downtoken.responseText);
													} catch (e) {
								...
								if (ctx) {
									...
									if (x_vars !== undefined
										...
										for ( var x_key in x_vars) {
											if (x_vars.hasOwnProperty(x_key)) {
												if (typeof x_vars[x_key] === 'function') {
													x_val = that
															.URLSafeBase64Encode(x_vars[x_key]
																	(up, file));
												} else if (typeof x_vars[x_key] !== 'object') {
													x_val = that
															.URLSafeBase64Encode(x_vars[x_key]);
									...
									var url = 'http://up.qiniu.com/mkfile/'
	...
	this.imageView2 = function(op, key) {
		var mode = op.mode || '', w = op.w || '', h = op.h || '', q = op.quality
				|| '', format = op.format || '';
		if (!mode) {
			return false;
		}
	...
	this.imageMogr2 = function(op, key) {
		var auto_orient = op['auto-orient'] || '', thumbnail = op.thumbnail
				|| '', strip = op.strip || '', gravity = op.gravity || '', crop = op.crop
				|| '', quality = op.quality || '', rotate = op.rotate || '', format = op.format
				|| '', blur = op.blur || '';
		// Todo check option
	...
	this.watermark = function(op, key) {
		...
		if (mode === 1) {
			...
			var text = op.text ? op.text : '', font = op.font ? op.font : '', fontsize = op.fontsize ? op.fontsize
		...
		var dissolve = op.dissolve || '', gravity = op.gravity || '', dx = op.dx
				|| '', dy = op.dy || '';
...
function up2Qiniu(pic, token, callback) {
	pic = pic.replace(/^data:\s*image\/\w+;base64,/mg, '');
	var key = randomString();
	var url = funnyConfig.up2QiniuUrl + Qiniu.URLSafeBase64Encode(key);
